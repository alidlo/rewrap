...
(defmethod parse-expr-output "array"
  [[_ & forms] f]
  `(cljs.core/array ~@(mapv f forms)))

(defmethod parse-expr-output "let"
  [[_ bindings & body] f]
...
(defmethod parse-expr-output "letfn*"
  [[_ bindings & body] f]
  `(letfn* ~bindings ~@(butlast body) ~(f (last body))))

(defmethod parse-expr-output "for"
  [[_ bindings body] f]
...
(defmethod parse-expr-output "if"
  [[_ condition a b] f]
  `(if ~condition ~(f a) ~(f b)))

(defmethod parse-expr-output "when"
  [[_ bindings & body] f]
  `(when ~bindings ~@(doall (for [x body] (f x)))))

(defmethod parse-expr-output "when-some"
  [[_ bindings & body] f]
  `(when-some ~bindings ~@(butlast body) ~(f (last body))))

(defmethod parse-expr-output "when-let"
  [[_ bindings & body] f]
  `(when-let ~bindings ~@(butlast body) ~(f (last body))))

(defmethod parse-expr-output "when-first"
  [[_ bindings & body] f]
  `(when-first ~bindings ~@(butlast body) ~(f (last body))))

(defmethod parse-expr-output "when-not"
  [[_ bindings & body] f]
  `(when-not ~bindings ~@(doall (for [x body] (f x)))))

(defmethod parse-expr-output "if-not"
  [[_ bindings & body] f]
  `(if-not ~bindings ~@(doall (for [x body] (f x)))))

(defmethod parse-expr-output "if-some"
  [[_ bindings & body] f]
  `(if-some ~bindings ~@(doall (for [x body] (f x)))))

(defmethod parse-expr-output "if-let"
  [[_ bindings & body] f]
  `(if-let ~bindings ~@(doall (for [x body] (f x)))))

(defmethod parse-expr-output "case"
  [[_ expr & clauses] f]
...
(defmethod parse-expr-output "condp"
...
(defmethod parse-expr-output "cond"
  [[_ & clauses] f]
  `(cond ~@(doall
            (mapcat
